...
#include "conveyor_gazebo_sim/ConveyorBeltState.h"
...
void ROSConveyorBeltPlugin::Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf)
{
  ...
  this->statePub = this->rosnode_->advertise<
    conveyor_gazebo_sim::ConveyorBeltState>(stateTopic, 1000);
}

/////////////////////////////////////////////////
void ROSConveyorBeltPlugin::Publish() const
{
  conveyor_gazebo_sim::ConveyorBeltState stateMsg;
...
bool ROSConveyorBeltPlugin::OnControlCommand(ros::ServiceEvent<
  conveyor_gazebo_sim::ConveyorBeltControl::Request, conveyor_gazebo_sim::ConveyorBeltControl::Response> & event)
{
  const conveyor_gazebo_sim::ConveyorBeltControl::Request& req = event.getRequest();
  conveyor_gazebo_sim::ConveyorBeltControl::Response& res = event.getResponse();
